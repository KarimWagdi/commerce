  ...
  static addCartItem = async (
    ...
    try {
      ...
      const cart = await CartRepository.findOne({
        where: { user_id: { id: request.user.id } },
      });
      const product = await productRepository.findOne({
        where: { id: request.body.product_id },
      });
      const isExist = await CartItemRepository.findOne({
        where: {
          cart_id: { id: +cart?.id },
          product_id: { id: request.body.product_id },
        },
      });
      if (!isExist) {
        console.log(1);
        ...
        const data = await CartItemRepository.findOne({
          where: {
            cart_id: { id: +cart?.id },
            product_id: { id: request.body.product_id },
          },
